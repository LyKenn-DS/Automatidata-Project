...
This project focuses on developing a robust linear regression model to understand and predict taxi fare amounts. Leveraging the comprehensive dataset, the supervised model was carefully constructed to not only provide precise fare estimates but also to identify as well as quantify the impact of key variables on the fare structure of New York City taxi cabs and for-hire vehicles.
